...
const fetchRegisterList = () => {
  getData('/api/register/register-list', (data) => {
...
//   // return value === "" || value === row.role;
// };
// const filteredRegisterList = computed(() => {
//   if (!roleFilter.value) {
//     return registerList.value;
//   } else {
//     return registerList.value.filter((row) => customFilterMethod(roleFilter.value, row));
//   }
// });
  ...
  <div>
    <el-table :data="registerList" empty-text="无申请" height="100%" stripe style="width: 100%">
      ...
      <el-table-column label="用户类型" prop="role">
        ...
        <!--        <template #header>-->
        <!--          <span>用户类型</span>-->
        <!--          <el-select v-model="roleFilter" placeholder="筛选" style="width: 60%">-->
